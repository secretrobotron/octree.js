define(function () {
  ...
  function Tree(options){
    ...
    var size = this.size = options.size || 0;
  ...
  Tree.prototype.insert = function(node){
    ...
    if (tNW && tNE && bNW && bNE && tSW && tSE && bSW && bSE) {
      ...
      var newChildren = [];
      if (tNW) newChildren[T_NW] = [x - offset, y - offset, z - offset];
      if (tNE) newChildren[T_NE] = [x + offset, y - offset, z - offset];
      if (bNW) newChildren[B_NW] = [x - offset, y + offset, z - offset];
      if (bNE) newChildren[B_NE] = [x + offset, y + offset, z - offset];
      if (tSW) newChildren[T_SW] = [x - offset, y - offset, z + offset];
      if (tSE) newChildren[T_SE] = [x + offset, y - offset, z + offset];
      if (bSW) newChildren[B_SW] = [x - offset, y + offset, z + offset];
      if (bSE) newChildren[B_SE] = [x + offset, y + offset, z + offset];
      ...
      // TODO: this is wrong. linkInsertion does too much. Only need to set rootTree, not hook everything else up.
